...
// lm32_cache()
//
// Constructor for lm32_cache class. Validates parameters, allocates some
// space for the cache lines and invalidates the cache, ready for use.
//

lm32_cache::lm32_cache (const int bytes_per_line, 
                        ...
                        const uint32_t upper_limit)
{
    ...
    lm32_cache_invalidate();

    // Use the first way to start off with
    for (int idx = 0; idx < LM32_MAX_SETS; idx++)
    {
...
void lm32_cache::lm32_cache_invalidate (void)
{
...
// lm32_cache_access()
//
// All reads (and only reads) to cached regions should call this function with 
// the read address. Returns a non-zero value of a hit.
//

int lm32_cache::lm32_cache_access (const uint32_t rd_byte_addr)
{
    ...
    {
        return LM32_CACHE_MISS;
    }

    int hit = LM32_CACHE_MISS;
    ...
    {
        hit = LM32_CACHE_HIT_WAY1;
    }
    // Check way 2 if configured
    else if (ways == 2 && m[1][line_offset].valid && m[1][line_offset].addr == line_addr)
    {
        hit = LM32_CACHE_HIT_WAY2;
    }

    // Update cache if missed
    if (!hit) 
    {
        // Mark the line as valid, and store the address bits ascossiated with this line
